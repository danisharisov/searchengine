...
public class ApiController {
    ...
    @Operation(summary = "Get site statistic")
    ...
    public ResponseEntity<Object> stopIndexing() {
        if (indexingService.stopIndexing()) {
            ...
            return new ResponseEntity<>(new BadRequest(false,
                    "Индексация не остановлена, так как она не была запущена."), HttpStatus.BAD_REQUEST);
        }
    }
        ...
        if (request.isEmpty()) {
            ...
            if (!site.isEmpty()) {
                if (siteRepository.findByUrl(site) == null) {
                    return new ResponseEntity<>(new BadRequest(false, "Нужная страница не найдена"),
                            HttpStatus.BAD_REQUEST);
                } else {
    ...
    public ResponseEntity<Object> indexPage(@RequestParam(name = "url") String url) {
        if (url.isEmpty()) {
            ...
            if (indexingService.urlIndexing(url) == true) {
                log.info("Страница - " + url + " - добавлена для реиндексации");
                return new ResponseEntity<>(new Response(true), HttpStatus.OK);
            } else {
                log.info("Требуемая страница не из файла конфигурации");
                return new ResponseEntity<>(new BadRequest(false, "Требуемая страница не из файла конфигурации"),
                        HttpStatus.BAD_REQUEST);
            }
        }
    }
